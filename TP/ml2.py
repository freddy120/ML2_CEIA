...
print(model)
preprocessor_pipeline = pickle.load( open( "featurizer.p", "rb" ) )
...
while True:
    ...
    os.system("gsutil ls gs://ml2_bucket/heart_load >> input_files_names.txt")

    file_to_read = open("input_files_names.txt")

    for line in file_to_read:
        line = line.replace("\n", "")
        if line == 'gs://ml2_bucket/heart_load/':
            continue
        else:
            ...
            blob = bucket.blob(f'heart_load/{file_name}')

            df = pd.read_csv(line)
            
            df["HeartDisease"] = model.predict(pre_processing(df, preprocessor_pipeline, numeric_features))

            output_file = f'gs://ml2_bucket/heart_predictions/{file_name.split(".")[0] + "_predict.csv"}'
